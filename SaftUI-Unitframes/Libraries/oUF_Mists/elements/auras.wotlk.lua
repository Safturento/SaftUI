	...
	if(name) then
		...
		* show - indicates whether the aura button should be shown (boolean)
		...
		local data = {
			...
			nameplateShowPersonal = nameplateShowSelf,
			nameplateShowAll = nameplateShowAll,
			spellId = spellId,
			canApplyAura = canApply,
			isBossAura = isBossDebuff,
			isFromPlayerOrPlayerPet = casterIsPlayer,
			timeMod = timeMode,
		}

		local show = (element.FilterAura or customFilter) (element, unit, data)
		...
		if(show) then
			...
			* isStealable - whether the aura can be stolen or purged (boolean)
			...
			if(element.PostUpdateButton) then
				element:PostUpdateButton(button, unit, index, position, duration, expiration, debuffType, isStealable)
			end
...
local function UpdateAuras(self, event, unit)
	...
	if(auras) then
		...
		* to   - the offset of the last aura button to be (re-)anchored (number)
		...
		if(fromRange or auras.createdButtons > auras.anchoredIcons) then
			...
			* to   - the offset of the last aura button to be (re-)anchored (number)
			--]]
			(auras.SetPosition or SetPosition) (auras, fromRange or auras.anchoredIcons + 1, toRange or auras.createdButtons)
			auras.anchoredIcons = auras.createdButtons
		end
	...
	if(buffs) then
		...
		if(fromRange or buffs.createdButtons > buffs.anchoredIcons) then
			(buffs.SetPosition or SetPosition) (buffs, fromRange or buffs.anchoredIcons + 1, toRange or buffs.createdButtons)
			buffs.anchoredIcons = buffs.createdButtons
		end
	...
	if(debuffs) then
		...
		if(fromRange or debuffs.createdButtons > debuffs.anchoredIcons) then
			(debuffs.SetPosition or SetPosition) (debuffs, fromRange or debuffs.anchoredIcons + 1, toRange or debuffs.createdButtons)
			debuffs.anchoredIcons = debuffs.createdButtons
		end
