...
local function SkinSlot(ID, slotName)
	...
	if not equipSlot.ProfessionQualityOverlay then
		equipSlot.ProfessionQualityOverlay = equipSlot:CreateTexture(nil, "OVERLAY");
		equipSlot.ProfessionQualityOverlay:SetPoint("TOPLEFT", 0, 0);
		equipSlot.ProfessionQualityOverlay:SetDrawLayer("OVERLAY", 7);
	end
